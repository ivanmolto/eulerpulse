...
export default function BreakdownFees({ slug, column }: DuneDataProps) {
  const { data, isLoading, error } = useQuery({
    queryKey: ["breakdown-fees", slug],
  ...
  const transformedData = chartData.reduce(
    (acc: Record<string, Record<string, any>>, row: Record<string, any>) => {
      ...
      const day = new Date(dateOnly + "T00:00:00Z").toISOString().split("T")[0]; // Convert to YYYY-MM-DD format
